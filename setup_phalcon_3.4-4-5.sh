...
RUN apt-get update && apt-get install -y git unzip libpcre3-dev gcc make re2c
...
case $choice in
  1)
    ...
    ;;
  2)
    versions_to_install=("4")
    ;;
  3)
    versions_to_install=("5")
    ;;
  4)
    versions_to_install=("3.4" "4" "5")
    ;;
  *)
...
for version in "${versions_to_install[@]}"; do
  install_phalcon_version $version
done
